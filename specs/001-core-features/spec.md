...
- **FR-003**: システムはエディタ標準の入力インターフェースを使用してユーザーからのテキスト入力を受け付けなければならない。
- **FR-004**: システムは取得した情報（メタデータ、コード、コメント）を規定のMarkdownテンプレートに従って整形しなければならない。
...
- **FR-009**: システムはプロジェクト単位で注釈データをグループ化して保持しなければならない。
- **FR-010**: システムは注釈一覧を表示・編集するための専用バッファ（または編集可能なインターフェース）を提供しなければならない。
- **FR-011**: システムは一覧バッファでの削除操作を検知し、対応する注釈データを内部ストアおよびQuickfix Listから削除しなければならない。
